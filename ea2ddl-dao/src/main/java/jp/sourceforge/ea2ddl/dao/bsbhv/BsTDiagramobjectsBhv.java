...
/**
 * The behavior of t_diagramobjects that the type is TABLE. <br />
 * <pre>
 * [primary-key]
 *     
 * 
 ...
 */
public abstract class BsTDiagramobjectsBhv extends org.seasar.dbflute.bhv.AbstractBehaviorReadable {
     ...
     * @param cb The condition-bean of TDiagramobjects. (NotNull)
     * @param entityRowHandler The handler of entity row of TDiagramobjects. (NotNull)
     */
    public void selectCursor(TDiagramobjectsCB cb, EntityRowHandler<TDiagramobjects> entityRowHandler) {
        assertCBNotNull(cb); assertObjectNotNull("entityRowHandler<TDiagramobjects>", entityRowHandler);
     ...
     * <pre>
     * tDiagramobjectsBhv.scalarSelect(Date.class).max(new ScalarQuery(TDiagramobjectsCB cb) {
     ...
     */
    public <RESULT> SLFunction<TDiagramobjectsCB, RESULT> scalarSelect(Class<RESULT> resultType) {
        TDiagramobjectsCB cb = newMyConditionBean();
        cb.xsetupForScalarSelect();
        cb.getSqlClause().disableSelectIndex(); // for when you use union
        return new SLFunction<TDiagramobjectsCB, RESULT>(cb, resultType);
    }
    // ===================================================================================
    //                                                                    Pull out Foreign
    //                                                                    ================
    ...
    //                                                Select
    //                                                ------
    protected int delegateSelectCount(TDiagramobjectsCB cb) { return invoke(createSelectCountCBCommand(cb)); }
    protected void delegateSelectCursor(TDiagramobjectsCB cb, EntityRowHandler<TDiagramobjects> entityRowHandler)
    { invoke(createSelectCursorCBCommand(cb, entityRowHandler, TDiagramobjects.class)); }
    protected int doCallReadCount(ConditionBean cb) { return delegateSelectCount((TDiagramobjectsCB)cb); }
    protected List<TDiagramobjects> delegateSelectList(TDiagramobjectsCB cb)
    { return invoke(createSelectListCBCommand(cb, TDiagramobjects.class)); }
    @SuppressWarnings("unchecked")
    protected List<Entity> doCallReadList(ConditionBean cb) { return (List)delegateSelectList((TDiagramobjectsCB)cb); }
