...
/**
 * The behavior of t_attribute that the type is TABLE. <br />
 * <pre>
 * [primary-key]
 *     
 * 
 ...
 */
public abstract class BsTAttributeBhv extends org.seasar.dbflute.bhv.AbstractBehaviorReadable {
     ...
     * @param cb The condition-bean of TAttribute. (NotNull)
     * @param entityRowHandler The handler of entity row of TAttribute. (NotNull)
     */
    public void selectCursor(TAttributeCB cb, EntityRowHandler<TAttribute> entityRowHandler) {
        assertCBNotNull(cb); assertObjectNotNull("entityRowHandler<TAttribute>", entityRowHandler);
     ...
     * <pre>
     * tAttributeBhv.scalarSelect(Date.class).max(new ScalarQuery(TAttributeCB cb) {
     ...
     */
    public <RESULT> SLFunction<TAttributeCB, RESULT> scalarSelect(Class<RESULT> resultType) {
        TAttributeCB cb = newMyConditionBean();
        cb.xsetupForScalarSelect();
        cb.getSqlClause().disableSelectIndex(); // for when you use union
        return new SLFunction<TAttributeCB, RESULT>(cb, resultType);
    }
    // ===================================================================================
    //                                                                    Pull out Foreign
    //                                                                    ================
     ...
     * @param tAttributeList The list of tAttribute. (NotNull)
     * @return The list of foreign table. (NotNull)
     */
    public List<TObject> pulloutTObject(List<TAttribute> tAttributeList) {
        return helpPulloutInternally(tAttributeList, new InternalPulloutCallback<TAttribute, TObject>() {
            public TObject callbackGetForeignEntity(TAttribute entity) { return entity.getTObject(); } });
    }
    ...
    //                                                Select
    //                                                ------
    protected int delegateSelectCount(TAttributeCB cb) { return invoke(createSelectCountCBCommand(cb)); }
    protected void delegateSelectCursor(TAttributeCB cb, EntityRowHandler<TAttribute> entityRowHandler)
    { invoke(createSelectCursorCBCommand(cb, entityRowHandler, TAttribute.class)); }
    protected int doCallReadCount(ConditionBean cb) { return delegateSelectCount((TAttributeCB)cb); }
    protected List<TAttribute> delegateSelectList(TAttributeCB cb)
    { return invoke(createSelectListCBCommand(cb, TAttribute.class)); }
    @SuppressWarnings("unchecked")
    protected List<Entity> doCallReadList(ConditionBean cb) { return (List)delegateSelectList((TAttributeCB)cb); }
