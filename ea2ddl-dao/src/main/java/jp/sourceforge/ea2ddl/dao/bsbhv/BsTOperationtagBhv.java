...
/**
 * The behavior of t_operationtag that the type is TABLE. <br />
 * <pre>
 * [primary-key]
 *     
 * 
...
public abstract class BsTOperationtagBhv extends org.seasar.dbflute.bhv.AbstractBehaviorReadable {
     ...
     * @param cb The condition-bean of TOperationtag. (NotNull)
     * @param entityRowHandler The handler of entity row of TOperationtag. (NotNull)
     */
    public void selectCursor(TOperationtagCB cb, EntityRowHandler<TOperationtag> entityRowHandler) {
        assertCBNotNull(cb); assertObjectNotNull("entityRowHandler<TOperationtag>", entityRowHandler);
     ...
     */
    public <RESULT> SLFunction<TOperationtagCB, RESULT> scalarSelect(Class<RESULT> resultType) {
        TOperationtagCB cb = newMyConditionBean();
        cb.xsetupForScalarSelect();
        cb.getSqlClause().disableSelectIndex(); // for when you use union
        return new SLFunction<TOperationtagCB, RESULT>(cb, resultType);
    }
    // ===================================================================================
    //                                                                    Pull out Foreign
    //                                                                    ================
    ...
    //                                                Select
    //                                                ------
    protected int delegateSelectCount(TOperationtagCB cb) { return invoke(createSelectCountCBCommand(cb)); }
    protected void delegateSelectCursor(TOperationtagCB cb, EntityRowHandler<TOperationtag> entityRowHandler)
    { invoke(createSelectCursorCBCommand(cb, entityRowHandler, TOperationtag.class)); }
    protected int doCallReadCount(ConditionBean cb) { return delegateSelectCount((TOperationtagCB)cb); }
    protected List<TOperationtag> delegateSelectList(TOperationtagCB cb)
    { return invoke(createSelectListCBCommand(cb, TOperationtag.class)); }
    @SuppressWarnings("unchecked")
    protected List<Entity> doCallReadList(ConditionBean cb) { return (List)delegateSelectList((TOperationtagCB)cb); }
