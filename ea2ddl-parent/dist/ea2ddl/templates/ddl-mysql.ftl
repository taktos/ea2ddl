...
;
<#if table.primaryKey??>ALTER TABLE ${table.name} ADD CONSTRAINT PRIMARY KEY
	(<#list table.primaryKey.columnNameList as columnName>${columnName}<#if columnName_has_next>,</#if></#list>)
;
</#if>
<#list table.uniqueList as unique>ALTER TABLE ${table.name} ADD CONSTRAINT UNIQUE ${unique.name}
	(<#list unique.columnNameList as columnName>${columnName}<#if columnName_has_next>,</#if></#list>)
;
</#list>
